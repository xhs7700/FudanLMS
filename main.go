...
func AddBook(title, author, isbn string) error {
	var err error
	err = ISBNValidate(isbn)
	if err != nil {
		return err
	} //check whether isbn is correct.
...
func BorrowBook(id, isbn string, intime time.Time) error {
	...
	if _, ok = FindBook(isbn); ok == false {
		return fmt.Errorf("book(isbn=%s) not exist.", isbn)
	}
	if _, ok = FindBorRec(id, isbn); ok == true {
...
func SuspendCheck(id string) (bool, error) {
	var err error
	var BorRecList []BorRec
	BorRecList, err = OverdueCheck(id)
	if err != nil {
		return false, fmt.Errorf("SuspendCheck(id:%s):%v", id, err)
	}
	if len(BorRecList) > 3 {
		return true, nil
	} else {
