...
func TestQueryBook(t *testing.T){
    var tests=[]struct{
    ...
    }{
        {
            "追风筝的人","title",
            []Book{
                Book{"追风筝的人","卡勒德·胡赛尼","9787208061644"},
                Book{"追风筝的人","卡勒德.胡赛尼","9787208060625"},
            },
            nil,
        },
        {"abcd","isbn",nil,fmt.Errorf("notnil")},
        {
            ...
            []Book{
                Book{"1984","George_Orwell","9787567748996"},
                Book{"1984","George_Orwell","9787567748997"},
                Book{"Animal_Farm","George_Orwell","9787567743908"},
            },
            nil,
        },
    }
    for _,test:=range(tests){
        ...
        if len(gotbooks)!=len(books){
            ...
            sort.Slice(books,func(i,j int)bool{
                if books[i].ISBN<books[j].ISBN{return true}
                return false
            })
            sort.Slice(gotbooks,func(i,j int)bool{
                if gotbooks[i].ISBN<gotbooks[j].ISBN{return true}
                return false
            })
            isequal=true
            for i:=range(books){
...
func TestBorRecQuery(t *testing.T){
    var tests=[]struct{
        ...
        //{"30000000000",nil,fmt.Errorf("notnil")},
        {
            "18307130090",
            []BorRec{
                BorRec{"18307130090","9787567748997","1984",time.Date(2020,5,12,17,30,0,0,time.Local),time.Date(2020,6,11,17,30,0,0,time.Local),0},
                BorRec{"18307130090","9787208061644","追风筝的人",time.Date(2020,5,11,16,18,37,0,time.Local),time.Date(2020,7,1,16,18,37,0,time.Local),3},
            },
            nil,
        },
    }
    for _,test:=range(tests){
        ...
        if len(gotborrecs)!=len(borrecs){
            ...
            sort.Slice(borrecs,func(i,j int)bool{
                if borrecs[i].BookISBN<borrecs[j].BookISBN{return true}
                return false
            })
            sort.Slice(gotborrecs,func(i,j int)bool{
                if gotborrecs[i].BookISBN<gotborrecs[j].BookISBN{return true}
                return false
            })
            isequal=true
            for i:=range(borrecs){
